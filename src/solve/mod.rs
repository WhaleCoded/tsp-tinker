...
use crate::types::{
    ...
    get_tsp_problem_file_paths_by_sub_dir, TSPAlgorithm, TSPPackage,
};
...
pub fn solve_tsp(
    ...
    for sub_dir_path in sub_dir_paths {
        overall_progress_pb.set_message(format!(
            "Solving: {:?}",
            sub_dir_path.file_name().expect("No directory name found.")
        ));
